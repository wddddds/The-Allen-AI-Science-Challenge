...
def get_answer_tf(ans):
    # compute the tf_idf for each answer
...
def get_docstf_idf(dir_data):
    ...
    for fname in os.listdir(dir_data):
        ...
        docs_tf[fname] = dd
        # length[fname] = doc_length
        count += 1
        if count % 500 == 0:
          print 'processing ', count, 'th document(tfidf)'
    ...
    # average_length = total_doc_len/count

    return docs_tf, idf


def get_docs_importance_for_question(question, docs_tf, word_idf, max_docs = None):
...
def get_docs_importance_for_question_bm25(question, dosc_tf, word_idf,average_length,doc_length, max_docs = None):
    question_words = set(tokenize(question))
    # go through each article
    doc_importance = []
    ...
    for doc, doc_tf in dosc_tf.iteritems():
        ...
        for w in question_words:
            if w in doc_tf:
                doc_imp += doc_tf[w] * word_idf[w]
                doc_imp_bm25 += word_idf[w]*((doc_tf[w] * 2.5)/(doc_tf[w] + 1.5 * (1 - 0.75 + 0.75 *((doc_length[doc])/(average_length)))))

        doc_importance.append((doc, doc_imp_bm25))
